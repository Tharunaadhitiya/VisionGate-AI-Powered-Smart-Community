...
const TABLE = 'found_items';
const COLS = 'id AS _id, itemName, description, foundLocation, imageUrl, color, brand, status, reportedBy, matchedItemId, matchScore, returnedAt, createdAt, updatedAt';

async function populate(item) {
  if (!item) return item;
  const [reporter, matched] = await Promise.all([
    item.reportedBy ? User.findById(item.reportedBy) : null,
    item.matchedItemId ? (async () => { const LostItem = require('./LostItem'); return LostItem.findById(item.matchedItemId); })() : null,
  ]);
