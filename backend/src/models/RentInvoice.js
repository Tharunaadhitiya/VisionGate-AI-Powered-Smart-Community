...
const TABLE = 'rent_invoices';
const COLS = 'id AS _id, configId, residentId, amount, lateFee, dueDate, periodMonth, status, paidAt, paymentId, sentReminderDay2, sentReminderDay3, sentReminderDay4, sentReminderDay5, sentReminderDaily, sentOverdue, createdAt';

async function populate(inv) {
  if (!inv) return inv;
  const [resident, payment] = await Promise.all([
    inv.residentId ? User.findById(inv.residentId) : null,
    inv.paymentId ? Payment.findById(inv.paymentId) : null,
  ]);
  inv.resident = resident;
  inv.payment = payment;
  return inv;
}
