...
const TABLE = 'rent_configurations';
const COLS = 'id AS _id, user_id, monthly_rent, due_day, late_fee, start_date, is_active, created_by, created_at';

async function populate(c) {
  if (!c) return c;
  const [resident, creator] = await Promise.all([
    c.user_id ? User.findById(c.user_id) : null,
    c.created_by ? User.findById(c.created_by) : null,
  ]);
  c.resident = resident;
  c.creator = creator;
  return c;
