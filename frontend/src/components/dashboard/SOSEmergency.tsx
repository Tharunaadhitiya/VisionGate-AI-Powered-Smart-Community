...
export default function SOSEmergency({ onClose }: { onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      <motion.div variants={modalOverlay} initial="hidden" animate="visible" exit="exit" className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
        <motion.div className="absolute inset-0 bg-black/70" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} />
        <motion.div variants={modalContent} className="relative glass-card p-6 max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between mb-4">
            ...
            <motion.button whileTap={{ scale: 0.9 }} onClick={onClose} className="p-1.5 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800"><X className="w-5 h-5" /></motion.button>
          </div>
          ...
          {sending ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3 mb-4">
                {presetEmergencies.map((emergency) => {
                  ...
                  return (
                    <motion.button key={emergency.label} onClick={() => handlePresetSelect(emergency.label)} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              ...
              <motion.button onClick={() => setStep('custom')} whileHover={{ scale: 1.01 }} className="btn-secondary w-full">Type Custom Emergency</motion.button>
            </>
          ) : (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <motion.button onClick={handleCustomSend} disabled={!customMessage.trim()} whileTap={{ scale: 0.97 }} className="btn-danger flex-1">
