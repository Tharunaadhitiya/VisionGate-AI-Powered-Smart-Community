...
export default function UserDirectory({ onClose }: { onClose: () => void }) {
  ...
  const handleCharge = async (userId: string) => {
    if (!chargeForm.title.trim()) return toast.error('Charge title is required');
    if (!chargeForm.amount || parseFloat(chargeForm.amount) <= 0) return toast.error('Amount is required');
    if (!chargeForm.dueDate) return toast.error('Invalid due date');
    ...
    try {
      ...
      console.log('Charge API Response:', response);
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                ...
                <div className="flex items-center gap-2 text-xs">
                  ...
                  {isAdmin && u._id !== currentUser?._id && (
                    <>
                      <button onClick={() => { setChargeUser(chargeUser === u._id ? null : u._id); resetChargeForm(); setResetUser(null); }}
                      ...
                      <button onClick={() => { setResetUser(resetUser === u._id ? null : u._id); setResetForm({ email: u.email, password: '' }); setChargeUser(null); }}
                      ...
                      {u.role === 'resident' && (
                        <button onClick={() => { setRentUser(rentUser === u._id ? null : u._id); setRentForm({ amount: '', dueDay: '5', lateFee: '0', startDate: new Date().toISOString().split('T')[0] }); setChargeUser(null); setResetUser(null); }}
